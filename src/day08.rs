...
fn get_antinodes_count(antennas: &Vec<(i32, i32)>, positions: &mut HashSet<(i32, i32)>, xmax: usize, ymax: usize) {
    ...
    antennas.iter()
        .enumerate().for_each(|(i, (ax, ay))| {
            antennas.iter().enumerate().for_each(|(j, (bx ,by))| {
                if i == j {
                    return;
                }
...
fn get_antinodes_count2(antennas: &Vec<(i32, i32)>, positions: &mut HashSet<(i32, i32)>, xmax: usize, ymax: usize) {
    if antennas.len() == 1 {
        return;
    }
    ...
    antennas.iter()
        .enumerate().for_each(|(i, (ax, ay))| {
            antennas.iter().enumerate().for_each(|(j, (bx ,by))| {
                ...
                /* Add in the 2 antennas too */
                positions.insert((*ax, *ay));
                ...
                let mut mag = 1;
                loop {
                    let (aa_x, aa_y) = (ax - mag*delta_x, ay - mag*delta_y);
                    if !is_within_map(aa_x, aa_y, xmax, ymax) {
                        break;
                    }
                ...
                mag = 1;
                loop {
                    let (ab_x, ab_y) = (bx + mag*delta_x, by + mag*delta_y);
                    if !is_within_map(ab_x, ab_y, xmax, ymax) {
                        break;
                    }
...
pub fn part2() {
    ...
    let mut positions: HashSet<(i32, i32)> = HashSet::new();
    antennas.iter().for_each(|(_, v)| {
        get_antinodes_count2(v, &mut positions, xmax, ymax);
    });
